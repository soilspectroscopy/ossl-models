...
```
**Note: some soil properties were natural-log transformed to help models fitting.**
...
To remove baseline offset, additive/multiplicative scattering effects, and multicollinearity in spectra from multiple sources, [SNV](https://cran.r-project.org/web/packages/prospectr/vignettes/prospectr.html#scatter-and-baseline-corrections) preprocessing is used before PCA compression and model calibration:
...
For calibration model fitting we use the first 120 PCs. This method was first time introduced by @chang2001near, although the authors used only top 10 PCA components, here we use 120 to account for small variations of the spectra in small absorption features. We focus on target soil variables for which there is enough training data:
