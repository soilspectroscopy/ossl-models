...
for(i in 1:nrow(fitted.modeling.combinations)) {
  ...
  file.export <- paste0(dir, iexport_name, "/cvpred_", imodel_name, ".qs")
  file.export.alt <- gsub("\\.qs", "\\.rds", file.export)

  if(file.exists(file.export)){file.remove(file.export)}
  if(file.exists(file.export.alt)){file.remove(file.export.alt)}

  tryCatch(
    expr = {qsave(cv.export, file.export)},
    error = function(e){saveRDS(cv.export, file.export.alt)}
  )
  ...
  export.metrics <- paste0(dir, iexport_name, "/perfmetrics_", imodel_name, ".csv")
  ...
  if(grepl("log..", iexport_name)) {
    ...
    export.plot <- paste0(dir, iexport_name, "/valplot_", imodel_name, ".png")
