...
## Re-exporting plots due to export inconsistencies
...
all.plots <- grep("cubist", all.plots, value = T)
...
metrics.files <- grep("cubist", metrics.files, value = T)

performance <- metrics.files %>%
  purrr::map_dfr(.f = read_csv, show_col_types = FALSE)

performance <- fitted.modeling.combinations %>%
  select(soil_property, model_name, description) %>%
